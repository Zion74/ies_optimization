...
class OperationModel:
    ...
    # 结果展示
    def result_process(self, bus_name):
        results = self.energy_system.results["main"]
        # 获取需要展示的节点
        ...
        for flow in ele_flows:
            if flow[0][0] != bus_name:
                ax.bar(self.date_time_index, show_bus["sequences"][flow],
                       0.03, bottom=bottom1, label=flow[0][0] + ' to ' + flow[0][1])
                bottom1 = [(a + b) for a, b in zip(show_bus["sequences"][flow], bottom1)]
            else:
                bottom2 = [(a - b) for a, b in zip(bottom2, show_bus["sequences"][flow])]
                ax.bar(self.date_time_index, show_bus["sequences"][flow],
        ...
        ax.set_yticks(np.linspace(min(bottom2)+0.1*min(bottom2), max(bottom1)+0.1*max(bottom1), 13, endpoint=True))
        plt.xlabel('t/h')
        plt.ylabel('P/kW')
        plt.show()
        # 输出求解结果
