...
def subAimFunc(args):
    ...
    time_start = 0
    is_success = True
    for month_day_index in range(len(month_days)):
        time_step = 24 * month_days[month_day_index]
        ele_price = [0.0028 for _ in range(time_step)]
        ...
        for t in range(time_start, time_start + time_step):
            ele_load[t - time_start] = operation_list[t][0]
            heat_load[t - time_start] = operation_list[t][1]
            cool_load[t - time_start] = operation_list[t][2]
            solar_radiation_list[t - time_start] = operation_list[t][3]
            wind_speed_list[t - time_start] = operation_list[t][4]
            temperature_list[t - time_start] = operation_list[t][5]
        # 计算风光出力
        ...
        try:
            oc += operation_model.get_objective_value()
        ...
        for i in range(time_step):
            if complementary_results["grid"][i] >= complementary_results["electricity overflow"][i]:
                net_ele_load[time_start+i] = complementary_results["grid"][i]
            else:
                net_ele_load[time_start + i] = 0 - complementary_results["electricity overflow"][i]
            if complementary_results["heat source"][i] >= complementary_results["heat overflow"][i]:
                net_heat_load[time_start+i] = complementary_results["heat source"][i]
            else:
                net_heat_load[time_start + i] = 0 - complementary_results["heat overflow"][i]
            if complementary_results["cool source"][i] >= complementary_results["cool overflow"][i]:
                net_cool_load[time_start+i] = complementary_results["cool source"][i]
            else:
                net_cool_load[time_start + i] = 0 - complementary_results["cool overflow"][i]
